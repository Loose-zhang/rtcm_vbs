...
| 双基站 A 优先合并 | `--dual`，A/B 历元对齐；共享卫星保持 A 为相位参考，B 补充 A 缺失卫星 |
...
| 双基站合并 | 有（`dual` 模式） | 有（A 优先相位参考 + B 补充缺失卫星 + 历元对齐） |
...
B 站载波相位会在有共同卫星/共同信号时估计 B→A 的相位偏置。B-only 卫星会尽量归一到 A 的参考系；如果某个信号已有重叠历史但当前偏置不可信，会抑制该信号的载波相位，避免向输出注入不连续相位。
...
| `CNR chose_A / chose_B` | 日志字段名沿用旧称；当前实现中共享卫星计入 `chose_A`，B-only 补充卫星计入 `chose_B` |
