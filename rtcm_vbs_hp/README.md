...
两个物理基站合成一个 VBS，每颗卫星从平均 SNR 更高的一侧选观测。**1005/1006 位置锚点以 A 站为准**，VBS 相对 A 偏移或使用绝对坐标均可；B 站的 1005/1006 被忽略以避免位置跳变。
...
[stat] 1005/6=11 MSM=104 | obs in=2993 out=2993 | sat corr=2993 no_eph=0 \
       | merge A=2 B=1 both_sat=780 epochs=98 | SSR fwd=37 | clients=1
```

| 字段 | 含义 |
|---|---|
| `1005/6` | 已输出的 1005/1006 帧数 |
| `MSM` | 已输出的 MSM 帧数（多星座拆分后）|
| `obs in / out` | 进入修正的卫星数 / 成功输出的卫星数 |
| `sat corr` | 应用了 VBS 修正的累计卫星次数 |
| `no_eph` | 因缺星历被丢弃的卫星次数（> 0 说明星历流不健全）|
| `merge A / B` | 合并时仅 A 或仅 B 参与的历元数（双基站）|
| `both_sat` | 同卫星两侧都有、做了 CNR 比较的次数 |
| `epochs` | 合并完成的历元数 |
| `SSR fwd` | 已透传的 SSR 帧数 |
...
3. **双基站**合并时 1005/1006 以 A 站为锚点；B 站的 1005/1006 被忽略。
