...
| 相位重编码 | `gen_rtcm3()`，输出真 RTCM3 MSM4 / MSM5 / MSM6 / MSM7，无 fine_pr / fine_ph 量化溢出 |
| 双基站 A 主站辅助改正 | `--dual`，A/B 历元对齐；外送 MSM 始终使用 A 观测，B 仅作内部辅助改正源 |
| 远距电离层/网络插值 | `--iono`，基于 A/B 同卫星伪距残差减几何差的结果，按 VBS 到 A/B 的距离权重插值 |
| SSR 改正与透传 | 可选 SSR 流用于 VBS 卫星位置计算；同时 1057–1068 / 1240–1270 / 11–14 等直接帧级转发 |
...
| `--humi H`                         | 对流层模型相对湿度，范围 `0..1`（默认 `0.7`）|
| `--iono`                          | 启用双基站电离层/网络残差插值 |
| `--msm N`                         | 输出 MSM 子类型：`4`、`5`、`6` 或 `7`（默认 `7`）|
| `--no-eph`                        | 不再次发送星历消息 |
| `--no-ssr`                        | 不转发 SSR 消息 |
...
| **MSM4 / MSM5 / MSM6 / MSM7 (全星座)** | 解码 → VBS 修正 → `gen_rtcm3(MSM4/5/6/7)` 按星座拆分编码后发送 |
| **MSM1 / MSM2 / MSM3** | 被 RTKLIB 解码为 obs；输出端统一以配置的 MSM4/5/6/7 重新编码 |
...
```text
rtcm_vbs_hp netfix-20260427: A=127.0.0.1:50001 -> out=:50002 | mode=absolute-LLH | trop=1 humi=0.70 | iono=1 | MSM=7 | SSR fwd=1
...
4. **SSR** 用于 VBS 卫星位置计算并按帧透传；SSR 消息本身不按 VBS 位置重写。
...
| `gen_rtcm3()` | 输出 MSM4/5/6/7、1005/1006、1019/1020/1042/1045/1046 编码 |
