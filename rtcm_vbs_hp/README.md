...
| A、B 历元时间差 > `--pair-tol-ms` | 先输出较旧的那一帧，不丢历元 |
...
| `--window MS`                     | 双基站历元对齐窗口（默认 `40` ms；仅一侧到达时最多等待这么久再单侧输出）|
| `--pair-tol-ms MS`                | 双基站 A/B 观测时间戳允许的最大相差（默认 `50` ms，旧版硬编码为 `10` ms；`1–250`，过大可能把相邻历元误配成一对）|
