...
typedef struct {
    const char *src_host; int src_port; int dual; const char *b_host; int b_port; int epoch_window_ms; int pair_tol_ms; int partner_dead_ms;
...
typedef struct { tcp_client_t tcp; rtcm_t rtcm; const char *host; int port; const char *tag; } channel_t;
...
static void channel_init(channel_t*c,const char*h,int p,const char*t){c->tcp.fd=-1;c->host=h;c->port=p;c->tag=t;init_rtcm(&c->rtcm);c->rtcm.time.time=time(NULL);c->rtcm.time.sec=0.0;}
static void channel_close(channel_t*c){tcpc_close(&c->tcp);free_rtcm(&c->rtcm);} 
static void channel_try_connect(channel_t*c){if(c->tcp.fd<0&&tcpc_connect(&c->tcp,c->host,c->port)==0)fprintf(stderr,"[%s] connected %s:%d\n",c->tag,c->host,c->port);} 
