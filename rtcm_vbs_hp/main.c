...
typedef struct {
    ...
    const char *eph_host; int eph_port; const char *ssr_host; int ssr_port; int out_port, out_staid_cfg;
...
static void usage(const char*p){fprintf(stderr,"Usage: %s [options]\n--dual --b-port PORT --staid ID --iono --trop --offset N E U | --vbs LAT LON ALT\n",p);}
static void run(const config_t*cfg){if(cfg->trace_file)traceopen(cfg->trace_file);tracelevel(cfg->trace_level);channel_t chA,chB,chEph,chSsr;channel_init(&chA,cfg->src_host,cfg->src_port,"A");if(cfg->dual)channel_init(&chB,cfg->b_host,cfg->b_port,"B");if(cfg->eph_port)channel_init(&chEph,cfg->eph_host,cfg->eph_port,"eph");if(cfg->ssr_port)channel_init(&chSsr,cfg->ssr_host,cfg->ssr_port,"ssr");rtcm_t rout;init_rtcm(&rout);rout.time.time=time(NULL);vbs_ctx_t vbs;double vbs_llh[3]={cfg->vbs_lat,cfg->vbs_lon,cfg->vbs_alt};vbs_init(&vbs,cfg->use_llh_offset,cfg->dn,cfg->de,cfg->du,cfg->use_llh_offset?NULL:vbs_llh,cfg->apply_trop);merger_t mrg;merger_init(&mrg,cfg->epoch_window_ms);net_interp_t neti;net_interp_init(&neti,cfg->iono_interp);tcp_server_t*srv=tcps_create(cfg->out_port,16);if(!srv){fprintf(stderr,"[error] cannot bind output port %d\n",cfg->out_port);return;}int subtype=cfg->msm_out_type?cfg->msm_out_type:7;long last_stat_t=(long)time(NULL),ssr_fwd=0;obsd_t last_a[MAXOBS],last_b[MAXOBS];int last_a_n=0,last_b_n=0;gtime_t last_a_t={0},last_b_t={0};int side_staid[2]={0,0};int out_staid=cfg->out_staid_cfg;double last_sat_extra_a[MAXSAT]={0},last_sat_extra_b[MAXSAT]={0};long last_net_ms=0;int have_net_corr=0;uint8_t buf[4096];while(!g_stop){channel_try_connect(&chA);if(cfg->dual)channel_try_connect(&chB);if(cfg->eph_port)channel_try_connect(&chEph);if(cfg->ssr_port)channel_try_connect(&chSsr);int any_data=0;channel_t*chs[4]={&chA,NULL,NULL,NULL};int nch=1;if(cfg->dual)chs[nch++]=&chB;if(cfg->eph_port)chs[nch++]=&chEph;if(cfg->ssr_port)chs[nch++]=&chSsr;for(int ci=0;ci<nch;ci++){channel_t*ch=chs[ci];if(ch->tcp.fd<0)continue;for(;;){int r=tcpc_read(&ch->tcp,buf,sizeof(buf));if(r==TCPC_AGAIN)break;if(r==0||r<0){if(r!=0||ch->tcp.fd>=0)fprintf(stderr,"[%s] disconnected\n",ch->tag);tcpc_close(&ch->tcp);break;}any_data=1;for(int i=0;i<r;i++){int ret=input_rtcm3(&ch->rtcm,buf[i]);if(!ret)continue;int mtype=frame_mtype(ch->rtcm.buff,ch->rtcm.len+3);if(is_ssr_msg(mtype)){if(cfg->ssr_passthrough){tcps_broadcast(srv,ch->rtcm.buff,ch->rtcm.len+3);ssr_fwd++;}continue;}rout.time=ch->rtcm.time;rout.staid=ch->rtcm.staid;if(cfg->out_staid_cfg)rout.staid=cfg->out_staid_cfg;if(cfg->dual&&ch==&chA&&ch->rtcm.staid&&!out_staid)out_staid=ch->rtcm.staid;if(cfg->dual&&out_staid)rout.staid=out_staid;if(ret==1){mirror_glo_fcn(&rout,&ch->rtcm);if(cfg->dual&&(ch==&chA||ch==&chB)){int side=(ch==&chA)?0:1;side_staid[side]=ch->rtcm.staid;if(side==0&&side_staid[0]&&!out_staid)out_staid=side_staid[0];merger_stash(&mrg,side,ch->rtcm.obs.data,ch->rtcm.obs.n,ch->rtcm.time,now_ms_mono());if(!side){copy_obs(last_a,&last_a_n,ch->rtcm.obs.data,ch->rtcm.obs.n);last_a_t=ch->rtcm.time;}else{copy_obs(last_b,&last_b_n,ch->rtcm.obs.data,ch->rtcm.obs.n);last_b_t=ch->rtcm.time;}}else if(ch==&chA){int n=ch->rtcm.obs.n>MAXOBS?MAXOBS:ch->rtcm.obs.n;memcpy(rout.obs.data,ch->rtcm.obs.data,n*sizeof(obsd_t));rout.obs.n=n;vbs_correct_obs(&vbs,&rout,NULL);if(rout.obs.n>0)emit_msm(&rout,srv,subtype);}}else if(ret==2){mirror_eph(&rout,&ch->rtcm);if(cfg->eph_out_enable)emit_eph(&rout,srv,ch->rtcm.ephsat);}else if(ret==5){int side=(ch==&chA)?0:((cfg->dual&&ch==&chB)?1:-1);if(side>=0){side_staid[side]=ch->rtcm.staid;if(side==0&&side_staid[0]&&!out_staid)out_staid=side_staid[0];vbs_update_base(&vbs,side,&ch->rtcm.sta);if(norm(vbs.vbs_ecef,3)>=1e3&&(!cfg->dual||side==0)&&(!cfg->dual||out_staid)){rout.staid=cfg->out_staid_cfg?cfg->out_staid_cfg:(cfg->dual?out_staid:ch->rtcm.staid);rout.sta=ch->rtcm.sta;vbs_rewrite_station(&vbs,&rout);emit_station(&rout,srv);}}}}}}if(cfg->dual){obsd_t raw_a[MAXOBS],raw_b[MAXOBS];int na=0,nb=0,have_a=0,have_b=0;gtime_t mt;double sat_extra_a[MAXSAT]={0},sat_extra_b[MAXSAT]={0};long poll_ms=now_ms_mono();if(merger_poll_pair(&mrg,poll_ms,raw_a,&na,&have_a,raw_b,&nb,&have_b,&mt)&&(have_a||have_b)){if(cfg->iono_interp&&have_a&&have_b){int nc=net_interp_build_dual(&neti,&vbs,&rout,raw_a,na,raw_b,nb,sat_extra_a,sat_extra_b);if(nc>0){memcpy(last_sat_extra_a,sat_extra_a,sizeof(last_sat_extra_a));memcpy(last_sat_extra_b,sat_extra_b,sizeof(last_sat_extra_b));last_net_ms=poll_ms;have_net_corr=1;}}else if(cfg->iono_interp&&have_net_corr){long age=poll_ms-last_net_ms;if(age>=0&&age<35000){double scale=age<5000?1.0:(double)(35000-age)/30000.0;for(int si=0;si<MAXSAT;si++){sat_extra_a[si]=last_sat_extra_a[si]*scale;sat_extra_b[si]=last_sat_extra_b[si]*scale;}}else have_net_corr=0;}static unsigned char side_a[MAXOBS],side_b[MAXOBS];if(have_a){memset(side_a,0,sizeof(side_a));memcpy(rout.obs.data,raw_a,na*sizeof(obsd_t));rout.obs.n=na;rout.time=mt;vbs_correct_obs_ex(&vbs,&rout,side_a,cfg->iono_interp?sat_extra_a:NULL);na=rout.obs.n;memcpy(raw_a,rout.obs.data,na*sizeof(obsd_t));if(na==0)have_a=0;}if(have_b){memset(side_b,1,sizeof(side_b));memcpy(rout.obs.data,raw_b,nb*sizeof(obsd_t));rout.obs.n=nb;rout.time=mt;vbs_correct_obs_ex(&vbs,&rout,side_b,cfg->iono_interp?sat_extra_b:NULL);nb=rout.obs.n;memcpy(raw_b,rout.obs.data,nb*sizeof(obsd_t));if(nb==0)have_b=0;}if(have_a||have_b){if(out_staid)rout.staid=out_staid;else if(have_a&&side_staid[0]){rout.staid=side_staid[0];out_staid=side_staid[0];}else rout.staid=0;int nm=merger_align_and_merge(&mrg,raw_a,na,have_a,raw_b,nb,have_b,rout.obs.data);rout.obs.n=nm;rout.time=mt;if(nm>0&&rout.staid)emit_msm(&rout,srv,subtype);}}}if(!any_data){sleep_ms(5);}long now_s=(long)time(NULL);if(now_s-last_stat_t>=15){if(cfg->dual)fprintf(stderr,"[stat] 1005/6=%ld MSM=%ld | obs in=%ld out=%ld | sat corr=%ld no_eph=%ld net=%ld | epochs merged=%ld A-only=%ld B-only=%ld | CNR both_sat=%ld chose_A=%ld chose_B=%ld | align init=%ld reset(slip=%ld code=%ld gap=%ld resid=%ld) | Bnorm=%ld Bunal=%ld switch_blk=%ld | iono epochs=%ld pairs=%ld interp=%ld rej=%ld | SSR fwd=%ld | clients=%d\n",vbs.n_frames_1005_6,vbs.n_frames_msm,vbs.n_obs_in,vbs.n_obs_out,vbs.n_sat_corrected,vbs.n_sat_no_eph,vbs.n_sat_netcorr,mrg.n_merged,mrg.n_a_only,mrg.n_b_only,mrg.n_both,mrg.n_chose_a,mrg.n_chose_b,mrg.n_align_init_ok,mrg.n_align_reset_slip,mrg.n_align_reset_code,mrg.n_align_reset_gap,mrg.n_align_reset_resid,mrg.n_b_norm_applied,mrg.n_b_unaligned_lli,mrg.n_switch_blocked,neti.n_epochs_used,neti.n_sat_pairs,neti.n_sat_interp,neti.n_sat_reject,ssr_fwd,tcps_num_clients(srv));else fprintf(stderr,"[stat] 1005/6=%ld MSM=%ld | obs in=%ld out=%ld | sat corr=%ld no_eph=%ld | SSR fwd=%ld | clients=%d\n",vbs.n_frames_1005_6,vbs.n_frames_msm,vbs.n_obs_in,vbs.n_obs_out,vbs.n_sat_corrected,vbs.n_sat_no_eph,ssr_fwd,tcps_num_clients(srv));last_stat_t=now_s;}}channel_close(&chA);if(cfg->dual)channel_close(&chB);if(cfg->eph_port)channel_close(&chEph);if(cfg->ssr_port)channel_close(&chSsr);tcps_destroy(srv);free_rtcm(&rout);}
int main(int argc,char**argv){config_t cfg={.src_host="127.0.0.1",.src_port=50001,.dual=0,.b_host="127.0.0.1",.b_port=50003,.epoch_window_ms=40,.eph_host="127.0.0.1",.eph_port=0,.ssr_host="127.0.0.1",.ssr_port=0,.out_port=50002,.out_staid_cfg=0,.use_llh_offset=1,.dn=0,.de=0,.du=0,.vbs_lat=0,.vbs_lon=0,.vbs_alt=0,.apply_trop=0,.iono_interp=0,.msm_out_type=7,.eph_out_enable=1,.ssr_passthrough=1,.trace_level=0,.trace_file=NULL};for(int i=1;i<argc;i++){if(!strcmp(argv[i],"--src-host")&&i+1<argc)cfg.src_host=argv[++i];else if(!strcmp(argv[i],"--src-port")&&i+1<argc)cfg.src_port=atoi(argv[++i]);else if(!strcmp(argv[i],"--dual"))cfg.dual=1;else if(!strcmp(argv[i],"--b-host")&&i+1<argc)cfg.b_host=argv[++i];else if(!strcmp(argv[i],"--b-port")&&i+1<argc)cfg.b_port=atoi(argv[++i]);else if(!strcmp(argv[i],"--window")&&i+1<argc)cfg.epoch_window_ms=atoi(argv[++i]);else if(!strcmp(argv[i],"--eph-host")&&i+1<argc)cfg.eph_host=argv[++i];else if(!strcmp(argv[i],"--eph-port")&&i+1<argc)cfg.eph_port=atoi(argv[++i]);else if(!strcmp(argv[i],"--ssr-host")&&i+1<argc)cfg.ssr_host=argv[++i];else if(!strcmp(argv[i],"--ssr-port")&&i+1<argc)cfg.ssr_port=atoi(argv[++i]);else if(!strcmp(argv[i],"--out-port")&&i+1<argc)cfg.out_port=atoi(argv[++i]);else if(!strcmp(argv[i],"--staid")&&i+1<argc)cfg.out_staid_cfg=atoi(argv[++i]);else if(!strcmp(argv[i],"--offset")&&i+3<argc){cfg.use_llh_offset=1;cfg.dn=atof(argv[++i]);cfg.de=atof(argv[++i]);cfg.du=atof(argv[++i]);}else if(!strcmp(argv[i],"--vbs")&&i+3<argc){cfg.use_llh_offset=0;cfg.vbs_lat=atof(argv[++i]);cfg.vbs_lon=atof(argv[++i]);cfg.vbs_alt=atof(argv[++i]);}else if(!strcmp(argv[i],"--trop"))cfg.apply_trop=1;else if(!strcmp(argv[i],"--iono"))cfg.iono_interp=1;else if(!strcmp(argv[i],"--msm")&&i+1<argc)cfg.msm_out_type=atoi(argv[++i]);else if(!strcmp(argv[i],"--no-eph"))cfg.eph_out_enable=0;else if(!strcmp(argv[i],"--no-ssr"))cfg.ssr_passthrough=0;else if(!strcmp(argv[i],"--trace")&&i+1<argc)cfg.trace_file=argv[++i];else if(!strcmp(argv[i],"--level")&&i+1<argc)cfg.trace_level=atoi(argv[++i]);else{usage(argv[0]);return 1;}}if(cfg.out_staid_cfg<0||cfg.out_staid_cfg>4095){fprintf(stderr,"[cfg] --staid must be 0..4095\n");return 1;}if(cfg.msm_out_type!=4&&cfg.msm_out_type!=7){fprintf(stderr,"[cfg] --msm must be 4 or 7\n");return 1;}if(cfg.iono_interp&&!cfg.dual){fprintf(stderr,"[cfg] --iono currently requires --dual\n");return 1;}setup_signals();fprintf(stderr,"rtcm_vbs_hp: A=%s:%d -> out=:%d | staid=%d | mode=%s | trop=%d | iono=%d | MSM=%d | SSR fwd=%d\n",cfg.src_host,cfg.src_port,cfg.out_port,cfg.out_staid_cfg,cfg.use_llh_offset?"NED-offset":"absolute-LLH",cfg.apply_trop,cfg.iono_interp,cfg.msm_out_type,cfg.ssr_passthrough);run(&cfg);return 0;}
