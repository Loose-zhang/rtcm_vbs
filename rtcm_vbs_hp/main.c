...
static void run(const config_t *cfg)
{
    ...
    net_interp_t neti;
    net_interp_init(&neti, cfg->iono_interp);
    ...
    long ssr_fwd = 0, ssr_dec = 0;
    int side_staid[2] = {0, 0};
    int out_staid = 0;
    ...
    while (!g_stop) {
        ...
        int any_data = 0;
        channel_t *chs[4] = {&chA, NULL, NULL, NULL};
        int nch = 1;
        if (cfg->dual) chs[nch++] = &chB;
        if (cfg->eph_port) chs[nch++] = &chEph;
        if (cfg->ssr_port) chs[nch++] = &chSsr;
        ...
        for (int ci = 0; ci < nch; ci++) {
            ...
            for (;;) {
                ...
                for (int i = 0; i < r; i++) {
                    ...
                    if (is_ssr_msg(mtype)) {
                        int nssr = mirror_ssr(&rout, &ch->rtcm);
                        if (nssr > 0) ssr_dec += nssr;
                    ...
                    if (cfg->dual && ch == &chA && ch->rtcm.staid) out_staid = ch->rtcm.staid;
                    if (cfg->dual && out_staid) rout.staid = out_staid;
                    ...
                    if (ret == 1) {
                        ...
                        if (cfg->dual && (ch == &chA || ch == &chB)) {
                            ...
                            merger_stash(&mrg, side, ch->rtcm.obs.data, ch->rtcm.obs.n,
                                         ch->rtcm.time, now_ms_mono());
                        } else if (ch == &chA) {
                            int n = ch->rtcm.obs.n > MAXOBS ? MAXOBS : ch->rtcm.obs.n;
                            memcpy(rout.obs.data, ch->rtcm.obs.data, n * sizeof(obsd_t));
                            rout.obs.n = n;
                        ...
                        if (cfg->eph_out_enable) emit_eph(&rout, srv, ch->rtcm.ephsat);
                    } else if (ret == 5) {
                        int side = (ch == &chA) ? 0 : ((cfg->dual && ch == &chB) ? 1 : -1);
                        if (side >= 0) {
                            side_staid[side] = ch->rtcm.staid;
                            if (side == 0 && side_staid[0]) out_staid = side_staid[0];
                            ...
                            if (norm(vbs.vbs_ecef, 3) >= 1e3 &&
                                (!cfg->dual || side == 0) &&
                                (!cfg->dual || out_staid)) {
                                rout.staid = cfg->dual ? out_staid : ch->rtcm.staid;
        ...
        if (cfg->dual) {
            obsd_t raw_a[MAXOBS], raw_b[MAXOBS];
            int na = 0, nb = 0, have_a = 0, have_b = 0;
            gtime_t mt;
            double sat_extra_m[MAXSAT] = {0};
            long poll_ms = now_ms_mono();

            if (merger_poll_pair(&mrg, poll_ms, raw_a, &na, &have_a,
                                  raw_b, &nb, &have_b, &mt) && (have_a || have_b)) {
                if (cfg->iono_interp && have_a && have_b) {
                    int nc = net_interp_build_dual(&neti, &vbs, &rout,
                                                   raw_a, na, raw_b, nb, sat_extra_m);
                    if (nc > 0) {
                        memcpy(last_sat_extra_m, sat_extra_m, sizeof(last_sat_extra_m));
                        last_net_ms = poll_ms;
                        have_net_corr = 1;
                    }
                } else if (cfg->iono_interp && have_net_corr) {
                    long age = poll_ms - last_net_ms;
                    if (age >= 0 && age < 35000) {
                        double scale = age < 5000 ? 1.0 : (double)(35000 - age) / 30000.0;
                        for (int si = 0; si < MAXSAT; si++) {
                            sat_extra_m[si] = last_sat_extra_m[si] * scale;
                        }
                    } else {
                        have_net_corr = 0;
                    }
                ...
                static unsigned char side_a[MAXOBS], side_b[MAXOBS];
                if (have_a) {
                    memset(side_a, 0, sizeof(side_a));
                    memcpy(rout.obs.data, raw_a, na * sizeof(obsd_t));
                    rout.obs.n = na;
                    rout.time = mt;
                    vbs_correct_obs_ex(&vbs, &rout, side_a,
                                       cfg->iono_interp ? sat_extra_m : NULL);
                    na = rout.obs.n;
                    memcpy(raw_a, rout.obs.data, na * sizeof(obsd_t));
                    if (na == 0) have_a = 0;
                }
                if (have_b) {
                    memset(side_b, 1, sizeof(side_b));
                    memcpy(rout.obs.data, raw_b, nb * sizeof(obsd_t));
                    rout.obs.n = nb;
                    rout.time = mt;
                    vbs_correct_obs_ex(&vbs, &rout, side_b,
                                       cfg->iono_interp ? sat_extra_m : NULL);
                    nb = rout.obs.n;
                    memcpy(raw_b, rout.obs.data, nb * sizeof(obsd_t));
                    if (nb == 0) have_b = 0;
                }
                if (have_a || have_b) {
                    if (out_staid) rout.staid = out_staid;
                    else if (have_a && side_staid[0]) {
                        rout.staid = side_staid[0];
                        out_staid = side_staid[0];
                    } else {
                        rout.staid = 0;
                    }
                    int nm = merger_align_and_merge(&mrg, raw_a, na, have_a,
                                                     raw_b, nb, have_b, rout.obs.data);
                    rout.obs.n = nm;
                    rout.time = mt;
                    if (nm > 0 && rout.staid) emit_msm(&rout, srv, subtype);
                }
            }
        }

        if (!any_data) sleep_ms(5);

        long now_s = (long)time(NULL);
        if (now_s - last_stat_t >= 15) {
            if (cfg->dual) {
                fprintf(stderr,
                    "[stat] 1005/6=%ld MSM=%ld | obs in=%ld out=%ld | sat corr=%ld no_eph=%ld net=%ld | SSR dec=%ld fwd=%ld used=%ld brdc_fallback=%ld | epochs merged=%ld A-only=%ld B-only=%ld | CNR both_sat=%ld chose_A=%ld chose_B=%ld | align init=%ld reset(slip=%ld code=%ld gap=%ld resid=%ld) | Bnorm=%ld Bunal=%ld switch_blk=%ld | iono epochs=%ld pairs=%ld interp=%ld rej=%ld | clients=%d\n",
                    vbs.n_frames_1005_6, vbs.n_frames_msm, vbs.n_obs_in,
                    vbs.n_obs_out, vbs.n_sat_corrected, vbs.n_sat_no_eph,
                    vbs.n_sat_netcorr, ssr_dec, ssr_fwd, vbs.n_sat_ssr_used,
                    vbs.n_sat_ssr_fallback, mrg.n_merged, mrg.n_a_only,
                    mrg.n_b_only, mrg.n_both, mrg.n_chose_a, mrg.n_chose_b,
                    mrg.n_align_init_ok, mrg.n_align_reset_slip,
                    mrg.n_align_reset_code, mrg.n_align_reset_gap,
                    mrg.n_align_reset_resid, mrg.n_b_norm_applied,
                    mrg.n_b_unaligned_lli, mrg.n_switch_blocked,
                    neti.n_epochs_used, neti.n_sat_pairs, neti.n_sat_interp,
                    neti.n_sat_reject, tcps_num_clients(srv));
            } else {
                fprintf(stderr,
                    "[stat] 1005/6=%ld MSM=%ld | obs in=%ld out=%ld | sat corr=%ld no_eph=%ld | SSR dec=%ld fwd=%ld used=%ld brdc_fallback=%ld | clients=%d\n",
                    vbs.n_frames_1005_6, vbs.n_frames_msm, vbs.n_obs_in,
                    vbs.n_obs_out, vbs.n_sat_corrected, vbs.n_sat_no_eph,
                    ssr_dec, ssr_fwd, vbs.n_sat_ssr_used,
                    vbs.n_sat_ssr_fallback, tcps_num_clients(srv));
            }
            last_stat_t = now_s;
        }
    }
